...
##### 🔹 QxAutoMLUser.h
This file contains the sensor configurations and API declarations of the static library, the content of configurations are automatical generated by customer sensor config in WebUI's data collection page.
...
```
4. Classify Interfaces Declaration
...
```
  ...
  Paramters:
     ...
     Each axis data takes 2 bytes space, 3-axis data take 6 bytes space, below is the example of 3-axis data type declaration:
   ```
   typedef struct
   {
     int16_t x;
